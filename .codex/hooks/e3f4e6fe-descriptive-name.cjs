// AUTO-GENERATED by claudekit ck migrate — PATCHED FOR CODEX v0.151.0
// Do not regenerate until ck migrate emits decision:"block" and additional_context for PreToolUse.
...
const ORIGINAL_HOOK = ".codex/hooks/descriptive-name.cjs";
...
/**
 * True when the given event's scrub rules allow a blocking decision.
 * Used to translate Claude Code's exit-code protocol (exit 2 = block) into
 * Codex's JSON protocol ({decision: "block"}).
 */
...
function main() {
  ...
  process.stdin.on("end", () => {
    ...
    // Translate only for events where the Codex capability table allows blocking.
    ...
    try {
      ...
      // (or stderr) as the block reason. Otherwise forward unchanged.
      if (isBlockSignal) {
    ...
    if (event === "PreToolUse" && sanitized?.permissionDecision === "deny") {
      sanitized.decision = "block";
      delete sanitized.permissionDecision;
    }
    ...
    if (
      ...
      typeof sanitized.hookSpecificOutput.additionalContext === "string"
    ) {
      process.stdout.write(JSON.stringify({
        additional_context: sanitized.hookSpecificOutput.additionalContext,
      }));
      process.exit(0);
    }

    // If the hook signalled block via exit 2 but didn't emit a block decision
    // in the JSON, translate — otherwise Codex ignores the exit code.
    if (
      isBlockSignal &&
      (!sanitized || sanitized.decision !== "block")
    ) {
      return emitDeny(stderrText.trim());
    }

    process.stdout.write(JSON.stringify(sanitized));
    // When the hook already emitted a valid block JSON but also exited 2,
    // exit 0 so Codex treats the block as authoritative (consistent with
